...
const ProposalResult = ({ title, totalVote, className, data }: any) => {
  ...
  const pageData = useMemo(() => {
    const res = (data?.voters || []).concat(
      [...Array(perPage * numSlide - (data?.voters?.length || 0))].map(
        function () {},
      ),
    );
    ...
    <Box className={className}>
      <Flex flexDirection={['column', 'row']} justifyContent={'space-between'}>
        ...
        <Text className={'side-total'} color={'#FFFFFF'}>
          No. of Supporters: {formatCurrency(data?.totalVoter, 0)}
        </Text>
        <Text className={'side-total'} color={'#FFFFFF'}>
          No. of TM Supported: {formatCurrency(data?.totalAmount, 0)} / {totalVote}
        </Text>
      </Flex>
      <Progress
        max={100}
                    ...
                    <InfoTooltip
                      label={
                        <Box>
                          <HorizontalItem
                            label={
                              <Text color={'rgba(255, 255, 255, 0.7)'}>Address</Text>
                            }
                            value={
                              <Text color={'#FFFFFF'}>
                                {shortenAddress(d.voter, 4, 4)}
                              </Text>
                            }
                          />
                          <HorizontalItem
                            label={
                              <Text color={'rgba(255, 255, 255, 0.7)'}>Amount</Text>
                            }
                            value={
                              <Text color={'#FFFFFF'}>
                                {formatCurrency(d.amount)} {'TM'}
                              </Text>
                            }
                          />
                        </Box>
                      ...
                      <Box key={d.voter} paddingX={4}>
                        ...
                        {compareString(account, d.voter) && (
                          <Text
                            ...
                            textAlign={'center'}
                            mt={-2}
                            w={px2rem(mobileScreen || tabletScreen ? 40 : 60)}
                          >
                            You
                          </Text>
                        )}
