    ...
    <form onSubmit={onSubmit} style={{ height: '100%' }}>
      ...
      />
      {
        isAuthenticated && new BigNumber(juiceBalance || 0).lte(0) && (
          <Text fontSize="xs" color="brand.warning.400" textAlign={"left"}>Your TC balance is insufficient. You can receive free TC on our faucet site <Link href={TRUSTLESS_FAUCET} target={"_blank"} style={{textDecoration: "underline"}}>here</Link>.</Text>
        )
      }
      {
        isAuthenticated && baseToken && BRIDGE_SUPPORT_TOKEN.includes(baseToken?.symbol) && new BigNumber(baseBalance || 0).lte(0) && (
          <Text fontSize="xs" color="brand.warning.400" textAlign={"left"}>Insufficient {baseToken?.symbol} balance! Consider swapping your {baseToken?.symbol?.replace('W', '')} to trustless network <Link href={TRUSTLESS_BRIDGE} target={"_blank"} style={{textDecoration: "underline"}}>here</Link>.</Text>
        )
      }
