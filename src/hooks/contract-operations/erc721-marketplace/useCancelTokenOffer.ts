...
const useCancelTokenOffer: ContractOperationHook<ICancelTokenOfferParams, Transaction | null> = () => {
  ...
  const call = useCallback(
    async (params: ICancelTokenOfferParams): Promise<Transaction | null> => {
      if (account && provider && contract) {
        ...
        // Call contract 
        const transaction = await contract.connect(provider.getSigner()).cancelMakeOffer(offerIdInBytes32, {
          from: account,
        });
        return transaction;
      }
