...
const useAcceptTokenOffer: ContractOperationHook<IAcceptTokenOfferParams, Transaction | null> = () => {
  ...
  const call = useCallback(
    async (params: IAcceptTokenOfferParams): Promise<Transaction | null> => {
      if (account && provider && contract) {
        ...
        // Call contract 
        const transaction = await contract.connect(provider.getSigner()).acceptMakeOffer(offerIdInBytes32, {
          from: account,
        });
        return transaction;
      }
